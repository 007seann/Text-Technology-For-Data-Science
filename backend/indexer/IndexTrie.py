...
class WordIndex:
    ...
    def search(self, word):
        ...
        for char in word:
            ...
            node = node.children[char]
        return (
            (word, node.index_id, node.word_frequency) if node.is_end_of_word else None
        )
    ...
    def visualize_word_index(
        ...
        def add_node(parent_id, char, node, postings):
            ...
            # Set color based on end of word
            node_colors.append(
                end_word_color if node.is_end_of_word else node_color
            )
        ...
        def traverse(node, parent_id=None, word=""):
            ...
            current_id = add_node(
                parent_id, word[-1] if word else "root", node, postings
            )
        ...
        for node_id, position in pos.items():
            ...
            plt.text(
                position[0],
                position[1] - posting_offset,
                "\n".join(postings),
                fontsize=posting_font_size,
                ha="center",
                va="center",
            )
...
if __name__ == "__main__":
    ...
    wordindex.insert("the", 5, test_ll)
    wordindex.insert("there", 6, test_ll)
    wordindex.insert("magic", 7, test_ll)
    wordindex.insert("magical", 8, test_ll)
